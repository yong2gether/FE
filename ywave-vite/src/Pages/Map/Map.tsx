...
export default function Map(): React.JSX.Element {
        ...
        (error) => {
          ...
          switch (error.code) {
            ...
            case error.POSITION_UNAVAILABLE:
              console.log(
                "❌ 위치 정보를 사용할 수 없습니다 (GPS 신호 약함 또는 차단됨)"
              );
  ...
  const getGoogleGeolocation = useCallback(async (): Promise<{
    position: LatLngLiteral;
    accuracy: number;
  } | null> => {
    if (!apiKey) {
          ...
          stores.map(async (store) => {
            try {
              ...
              return {
                ...
                category:
                  (store.category as MarkerCategory) ||
                  ("기타" as MarkerCategory),
                name: store.name,
                address: placeDetails.formattedAddress || store.sigungu,
                images: placeDetails.photos
                  ? placeDetails.photos.map((photo) => photo.url)
                  : [
  ...
  const getDistanceMeters = useCallback(
    (a: LatLngLiteral, b: LatLngLiteral): number => {
  ...
  const getCurrentLocation = useCallback(async () => {
    ...
    try {
      ...
      if (!gpsResult || gpsResult.accuracy >= 100) {
        ...
        if (googleResult) {
          console.log(
            "✅ Google Geolocation API 위치 사용:",
            googleResult.position,
            "정확도:",
            googleResult.accuracy,
            "m"
          );
      ...
      <MapWrapper>
        <GoogleMap
          ...
          onIdle={() => {
            ...
            if (!mc) return;
            const currentCenter: LatLngLiteral = {
              lat: mc.lat(),
              lng: mc.lng(),
            };
            // 사용자 드래그 후에만 판단
            if (userInteractingRef.current) {
              const base =
                lastSearchCenterRef.current ?? userPosition ?? center;
  ...
  return <PageContainer>{renderBody()}</PageContainer>;
}
